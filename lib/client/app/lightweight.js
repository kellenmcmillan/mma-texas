...
uuid){
    ...
    var videoBucket = firebaseStorage.ref().child('video');
    var fileBucket = firebaseStorage.ref().child('files');
    var imageDatabase = realtimeDatabase.ref().child('images');
    var videoDatabase = realtimeDatabase.ref().child('video');
    var fileDatabase = realtimeDatabase.ref().child('files');
    ...
    $rootScope.sendIndexLead = function(lead){
        var data = {
            name : $sanitize(lead.birthday),
            email : $sanitize(lead.firstname),
            message : $sanitize(lead.lastname)
        };

        $http({
            url: ("https://us-central1-mma-texas.cloudfunctions.net/sendFormLead"),
            method: 'POST',
            data: data
        })
    ...
    $rootScope.deleteMedia = function(image, index){
        ...
        firebaseStorage.ref("images/" + image_source).delete().then(function() {
