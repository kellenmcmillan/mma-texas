...
$http){
    ...
    ///////////////////////////////////////////////////// Compile blogs and Paginate
    blog_data.getBlogs().then(function(result){
        if(result.length > 0){
            $rootScope.bloglist = result;
            imagelist = result;
            $rootScope.blogPageSize = 12;
            $rootScope.currentBlogPage = 0;
            $rootScope.numberOfBlogPages = Math.ceil(result.length/$rootScope.blogPageSize);
        }
    });
    ///////////////////////////////////////////////////// Compile blogs and Paginate
    ...
    ///////////////////////////////////////////////////// blog Query
    $rootScope.queryBlogs = function(query){
        $rootScope.currentBlogPage = 0;
        $rootScope.bloglist = filterFilter($rootScope.bloglist,query);
        $rootScope.numberOfBlogPages = Math.ceil($rootScope.bloglist.length/$rootScope.blogPageSize);
    }
    $rootScope.clearQueryBlogs = function(){
        $rootScope.currentBlogPage = 0;
        $rootScope.queryFilter = undefined;
        $rootScope.bloglist = bloglist;
        $rootScope.numberOfBlogPages = Math.ceil($rootScope.bloglist.length/$rootScope.blogPageSize);
    }
    ///////////////////////////////////////////////////// blog Query
    ...
    $rootScope.deleteMedia = function(image, index){
        ...
        if (image.source){
            ...
            image_source = image.avatar.split('https://storage.googleapis.com/mma-texas.appspot.com/media/images/').pop();
        }
        imageStorage.ref('/' + image_source).delete().then(function() {
            imageDatabase.child(image.id).remove();
            $timeout(function(){
                $rootScope.$broadcast('server-event', {
                    data:{
    ...
    ///////////////////////////////////////////////////// Add Blog Post
    $rootScope.addBlog = function(type, data){
        ...
        var new_blog_post = {};
        new_blog_post.data = {};
        new_blog_post.data.page = {};
        new_blog_post.data.page.foreground = {};
        new_blog_post.data.page.foreground.images = {};
        new_blog_post.data.page.background = {};
        new_blog_post.data.page.background.images = {};
        new_blog_post.data.page.contents = [];
        new_blog_post.settings = {};
        new_blog_post.settings.configs = {};
        // configure data structure
        ...
        new_blog_post.data.title = data.title;
        new_blog_post.data.subtitle = data.subtitle;
        new_blog_post.data.headline = data.headline;
        new_blog_post.data.author = $rootScope.me.firstname + " " + $rootScope.me.lastname;
        new_blog_post.data.date = new Date();
        new_blog_post.data.page.foreground.images.source = imageObj.source;
        new_blog_post.data.page.contents.push(paragraphObj);
        new_blog_post.settings.url = "/blog/" + trim_title(data.title);
        new_blog_post.settings.page_type = "blog";
        new_blog_post.settings.name = data.title;
        new_blog_post.settings.locked = false;
        new_blog_post.settings.configs.add_content = true;
        new_blog_post.settings.configs.contentId = "blog";
        new_blog_post.settings.configs.has_title_subtitle = true;
        new_blog_post.settings.configs.index_content = false;
        new_blog_post.settings.configs.multiview = true;
        new_blog_post.settings.configs.mulitview_label = "Posts";
        new_blog_post.settings.configs.primary_image = true;
        new_blog_post.settings.configs.reorder_content = true;
        new_blog_post.settings.configs.title_view = true;
        $rootScope.pages.push(new_blog_post);
        var data = angular.toJson($rootScope.pages);
        var update = JSON.parse(data);
        appDataBucket.child("pages").set(update);
        $timeout(function(){
    ...
    $rootScope.parentNav = function(item){
        ...
        if (item.navigation.child) {
            $rootScope.child_nav_items = item.navigation.child.data;
        } else {
            var childNavItem = {};
            childNavItem.enabled = true
            childNavItem.name = "Enter Navigation Item Name";
            childNavItem.url = null;
            $rootScope.child_nav_items = [];
            $rootScope.child_nav_items.push(childNavItem);
        }
    }

    $rootScope.clearParentNav = function(){
        $rootScope.parentNavName = null;
        $rootScope.child_nav_items = null;
    }
    ...
    ///////////////////////////////////////////////////// Send Lead
    $rootScope.sendIndexLead = function(lead){
        var data = {
            name : lead.name,
            email : lead.email,
...
.factory('blog_data', function($q){
    var realtimeDatabase = firebase.database();
    var blogDatabase = realtimeDatabase.ref().child('blogs/posts');
    var getBlogs = function(){
        var defer = $q.defer();
        var bloglist = [];
        blogDatabase.once('value')
        .then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                var childKey = childSnapshot.key;
                var childData = childSnapshot.val();
                var childDataValue = childData;
                childDataValue.id = childKey;
                bloglist.push(childDataValue);
            });
            defer.resolve(bloglist); 
        });
        return defer.promise;
    }
    return {
