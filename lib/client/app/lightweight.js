...
$http){
    ...
    var daysOfTheWeek = function(day){
        switch(day){
            case "monday": 1;
                break;
            case "tuesday": 2;
    ...
    var quantifyFrequency = function(frequency){
        ...
        switch(frequency){
            case "daily": 86400000;
                break;
            case "weekly": 604800000;
                break;
            case "bi-weekly": 1209600000;
                break;
            case "monthly": 2628288000;
                break;
            default:
                break;
        }
    ...
    $rootScope.createTask = function(task){
        ...
        var theDateMilliseconds = theDate.now();
        var today = theDate.getDay();
        var frequency = quantifyFrequency(task.frequency);
        task.offset = 0;
        var startDayDraft = theDateMilliseconds + frequency;
        var targetDate = theDate.setTime(startDayDraft);
        var targetInitialDay = targetDate.getDay();
        console.log("target day initially " + daysOfTheWeek(targetInitialDay) + " target date initially " + targetDate);
        var tempDay = null;
        var t = 0;
        if(task.runDate){
            var runDate = new Date(task.runDate);
            task.runDate = runDate.getTime();
        }
        
        //how many days til day of week -- closest to frequency target
        task.created_on = Date.now();
        // create date obj for runtime and interval for next run.
        if(task.runDay && task.runTime && task.frequency){

            if(task.runDay !== targetInitialDay){
                for (var i = today;  i < runDayAlg.length; i++) {
                    
                    if (runDayAlg[i] == task.runDay){
                        task.offset = t;
                        task.runDay = targetInitialDay;
                    }
                    t++;
            ...
            task.offset = task.offset*86400000;

            task.next_runtime = startDayDraft + task.offset;
            console.log(theDate.setTime(task.next_runtime));
    ...
    $rootScope.updateTask = function(task){
        ...
        if(task.runDate){
            task.runDate = JSON.parse(JSON.stringify(new Date(task.runDate)));
        }
