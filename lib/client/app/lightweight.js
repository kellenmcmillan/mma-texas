...
$http){
    ...
    $rootScope.createTask = function(task){
        ...
        if (task.frequency){
            ...
            frequency = quantifyFrequency(task.frequency);
            startDayDraft = theDateMilliseconds + frequency;
            targetDate = theDate2.setTime(startDayDraft);
            runDay = task.runDay;

            console.log("target day initially " + runDay + " target date initially " + targetDate);
            ...
            if(t > 4){
                ...
                task.offset = offset*-86400000;
            } else {
                task.offset = offset*86400000;
            }
            var tempDate = new Date(startDayDraft);
            if (tempDate.getDay() !== runDay){
                //perform check like 2 previous if statements to get the amount of milliseconds to +/- from startDayDraft
            }
            task.next_runtime = startDayDraft + task.offset;
            var next_runtime = new Date(task.next_runtime);

            console.log("next scheduled run date " + next_runtime);
        }
