...
$http){
    ...
    $rootScope.rolelist = [];
    var userslist = [];
    if(!$rootScope.me.files){
        $rootScope.me.files = [];
    }
    if(!$rootScope.me.files_shared_with_me){
        $rootScope.me.files_shared_with_me = [];
    }
    if(!$rootScope.me.notifications){
        $rootScope.me.notifications = [];
    }
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        var fileObj = {};
                        fileObj.metadata = {};
                        var appended_cloud_source = "https://storage.googleapis.com/mma-texas.appspot.com/media/files/users/" + $rootScope.me.id + "/files/"
                        ...
                        $rootScope.me.files.unshift(file);
                    }
                );
            });
        });
    ...
    $rootScope.addPage = function(type, data){
        ...
        switch(type){
            ...
            case "project":
                ...
                $rootScope.updateTags(data.title);
