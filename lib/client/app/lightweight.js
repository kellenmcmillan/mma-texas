...
$http){
    ...
    $rootScope.createTask = function(task){
        ...
        if (task.frequency){
            ...
            if (frequency < 2628288000){
                var counter = 0;
                var taskDateVar = theDate.getTime() + frequency;
                var exact_interval_date = new Date(taskDateVar);
                var exact_interval_date_day_number = exact_interval_date.getDay();
                targetDay = task.runDay;
                
                var runDayAlgFunc = function(){
                    for (var i = exact_interval_date_day_number; i >= targetDay; i--) {
                        counter++;
                    }
                    if (counter > 3){
                        counter = 0;
                        for (var i = exact_interval_date_day_number; runDayAlg[i] !== targetDay; i++) {
                            counter++;
                        }
                        counter = counter + 1;
                        var randVar = -86400000*counter; 
                        task.next_runtime = taskDateVar + randVar; 
                    } else {
                        var randVar = 86400000*counter; 
                        task.next_runtime = taskDateVar + randVar; 
                    }
                ...
                task.next_runtime = theDate.getTime() + frequency
            }
        }
    ...
    $rootScope.updateTask = function(task){
        ...
        if (task.frequency){
            ...
            if (frequency < 2628288000){
                ...
                var runDayAlgFunc = function(){
                    for (var i = exact_interval_date_day_number; i >= targetDate; i--) {
                        counter++;
                    }
                    if (counter > 3){
                        counter = 0;
