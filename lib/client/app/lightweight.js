...
$window){
    ...
    this.reset_email = function(arg){
        ...
        $http({
            url: ("https://us-central1-mma-texas.cloudfunctions.net/updateEmail"),
            method: 'POST',
            data: data
        })
        .then(function success (response) {
    ...
    this.modify_project = function(arg){
        ...
        var data = angular.toJson($rootScope.pages[arg]);
        var update = JSON.parse(data);
        realtimeDatabase.ref().child('data').child("pages/" + arg).set(update);
        $timeout(function(){
           $rootScope.$broadcast('server-event', {
                data:{
    ...
    this.run_task = function(arg, event){
        ...
        if (event == undefined){
            ...
            $http({
                url: ("https://us-central1-mma-texas.cloudfunctions.net/app_task_endpoint"),
                method: 'POST',
                data: data
            })
            .then(function success (response) {
...
$http){
    ...
    $rootScope.signIn = function(arg){
        ...
        arg.email = JSON.stringify(arg.email);

        arg.password = JSON.stringify(arg.password);
        ...
        .catch(function(error) {
            ...
            console.log("Lightweight Log - Code: " + errorCode + " Message: " + errorMessage);
    ...
    $rootScope.addGalleryTemplate = function(){
        ...
        if (!$rootScope.galleries){
            $rootScope.galleries = [];
            $rootScope.galleries.push(galleryObj);
        } else {
    ...
    $rootScope.modifyProject = function(page_index){
        lightweight_firebase.modify_project(page_index);
    }
