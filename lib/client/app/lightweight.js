...
$http){
    ...
    $rootScope.createTask = function(task){
        ...
        var targetDate = theDate2.setTime(startDayDraft);
        var targetInitialDay = theDate.getDay(targetDate);
        console.log("target day initially " + daysOfTheWeek(targetInitialDay) + " target date initially " + targetDate);
        var tempDay = null;
        var t = 0;
        if(task.runDate){
            task.runDate = theDate.getTime(task.runDate);
        }
        ...
        if(task.runDay && task.runTime && task.frequency){
            ...
            console.log("next scheduled run date " + theDate3.setTime(task.next_runtime));
