...
$http){
    ...
    $rootScope.project_types = ["Commercial", "Education", "Energy", "Entertainment", "Government", "Healthcare", "Multi-Family", "Single Family"];
    ////////////////////////////////// Variables and scoped variables
    ...
    $rootScope.addUser = function(user){
        ...
        $http({
            url: ("https://us-central1-mma-texas.cloudfunctions.net/addUser"),
            method: 'POST',
            data: data
        })
    ...
    $rootScope.passReset = function(resetUser){
        ...
        $http({
            url: ("https://us-central1-mma-texas.cloudfunctions.net/updatePassword"),
            method: 'POST',
            data: data
        })
        .then(function success (response) {
    ...
    $rootScope.deleteMedia = function(image, index){
        ...
        image_source = image.source.split('https://storage.googleapis.com/mma-texas.appspot.com/media/images/').pop();
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        var fileObj = {};
                        fileObj.metadata = {};
                        var appended_cloud_source = "https://storage.googleapis.com/mma-texas.appspot.com/media/images/"
    ...
    var time_unit = 1800000;

    $rootScope.timeObject = [
        {"time": "6:30AM", "milliseconds_since_midnight": 7 * time_unit }, 
        {"time": "7:00AM", "milliseconds_since_midnight": 8 * time_unit }, 
        {"time": "7:30AM", "milliseconds_since_midnight": 9 * time_unit }, 
        {"time": "8:00AM", "milliseconds_since_midnight": 10 * time_unit }, 
        {"time": "8:30AM", "milliseconds_since_midnight": 11 * time_unit }, 
        {"time": "9:00AM", "milliseconds_since_midnight": 12 * time_unit }, 
        {"time": "9:30AM", "milliseconds_since_midnight": 13 * time_unit }, 
        {"time": "10:00AM", "milliseconds_since_midnight": 14 * time_unit }, 
        {"time": "10:30AM", "milliseconds_since_midnight": 15 * time_unit }, 
        {"time": "11:00AM", "milliseconds_since_midnight": 16 * time_unit }, 
        {"time": "11:30AM", "milliseconds_since_midnight": 17 * time_unit }, 
        {"time": "12:00PM", "milliseconds_since_midnight": 18 * time_unit }, 
        {"time": "12:30PM", "milliseconds_since_midnight": 19 * time_unit }, 
        {"time": "1:00PM", "milliseconds_since_midnight": 20 * time_unit }, 
        {"time": "1:30PM", "milliseconds_since_midnight": 21 * time_unit }, 
        {"time": "2:00PM", "milliseconds_since_midnight": 22 * time_unit }, 
        {"time": "2:30PM", "milliseconds_since_midnight": 23 * time_unit }, 
        {"time": "3:00PM", "milliseconds_since_midnight": 24 * time_unit }, 
        {"time": "3:30PM", "milliseconds_since_midnight": 25 * time_unit }, 
        {"time": "4:00PM", "milliseconds_since_midnight": 26 * time_unit },
        {"time": "4:30PM", "milliseconds_since_midnight": 27 * time_unit },
        {"time": "5:00PM", "milliseconds_since_midnight": 28 * time_unit }, 
        {"time": "5:30PM", "milliseconds_since_midnight": 29 * time_unit }, 
        {"time": "6:00PM", "milliseconds_since_midnight": 30 * time_unit }, 
        {"time": "6:30PM", "milliseconds_since_midnight": 31 * time_unit }, 
        {"time": "7:00PM", "milliseconds_since_midnight": 32 * time_unit }, 
        {"time": "7:30PM", "milliseconds_since_midnight": 33 * time_unit }, 
        {"time": "8:00PM", "milliseconds_since_midnight": 34 * time_unit }
    ]

    $rootScope.app_triggers = [
        "User's First 30 Second On App",
        "First New View Loaded",
        "Inactive For 60 Seconds",
        "Form Started But Not Completed",
        "Projects Page Viewed",
        "Services Page Viewed",
        "Blog Page Visited",
        "User's First Login",
        "Shared Page Viewed",
        "User Account Manually Created"
    ];
    ...
    var initDate = function(){
        ...
        var date_var2 = new Date();
        var date_var3 = new Date();
    ...
    $rootScope.createTask = function(task){
        ...
        // Trigger type 3
        if (task.isReoccuring){

            // task.reoccuring_day
            // task.reoccuring_frequency

            if (task.reoccuring_week){
                var working_week = [];
                for (var i = 0; i < this_month.length; i++) {
                    var this_day = this_month[i];
                    if(this_day.week === task.run_week && this_day.day === task.run_day){
                        task.next_runtime = task.run_clock_time.milliseconds_since_midnight + this_day.milliseconds;
                        var next_date = new Date(task.next_runtime);
                        $timeout(function(){
                            $rootScope.$broadcast('server-event', {
                                data:{
                                    message: "Next Run Time " + next_date
                                }
                            });
                        }, 500);
                    }
                }
                console.log("working week contents " + JSON.stringify(working_week));
                // check for matching day
                // if matching date is in the past
                // if not save as next runtime
                // else
                // angular foreach the next month
                // check for matching week
                // check for matching day
                // set the day
                // get the milliseconds
                // add task time in milliseconds
                // save as next runtime
            }

            if (!task.reoccuring_date && task.reoccuring_week){
    ...
    $rootScope.addPage = function(type, data){
        ...
        var imageObj = {};
        imageObj.source = "https://storage.googleapis.com/mma-texas.appspot.com/media/images/missing-image-wide.png";
    ...
    $rootScope.sendLead = function(lead, source){
        ...
        $http({
            url: ("https://us-central1-mma-texas.cloudfunctions.net/lead"),
            method: 'POST',
            data: data
        })
        .then(function success (response) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        var fileObj = {};
                        fileObj.metadata = {};
                        var appended_cloud_source = "https://storage.googleapis.com/mma-texas.appspot.com/media/files/" + $rootScope.me.id + "/files/"
                        fileObj.source = appended_cloud_source + task.snapshot.metadata.name;
                        fileObj.id = uuid.v4();
