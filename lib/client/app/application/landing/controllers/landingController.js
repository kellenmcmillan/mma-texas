...
$window
    ...
    // Slider Logic
    $scope.number_of_posts__news = 6;
    // Slider Logic

    var calculate_virtual_panels = function(items, defaultNumberOfItems){
    	var numberOfPanels = Math.floor(items/defaultNumberOfItems)+Math.ceil(items%2);
    ...
    var news_post_window = $timeout(function(){
    	if($rootScope.window_width > 840){
    		$scope.news_post_width = calculate_virtual_panels(4,3);
    	} else {
    		$scope.news_post_width = calculate_virtual_panels(4,1);
    	}
    }, 1000);
