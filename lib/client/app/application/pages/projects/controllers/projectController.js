    ...
    var load_page = function(){
        for(let i = 0, l = $rootScope.pages.length; i < l && continue_looping == true; i++) {
            ...
            if(url == pageId){
                ...
                angular.forEach($rootScope.imagelist, function(image){
                    if(image.metadata != undefined){
                        if(image.metadata.tags != undefined){
                            for (var i = 0; i <= image.metadata.tags.length; i++) {
                                console.log("checking tags for projects " + image.metadata.tags[i]);
                                if (image.metadata.tags[i] == $rootScope.projectPage.data.title){
                                    $rootScope.projectImages.push();
                                }
                            } 
                        }
                    }
