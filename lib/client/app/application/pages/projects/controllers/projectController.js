        ...
        for(let i = 0, l = $rootScope.pages.length; i < l && continue_looping == true; i++) {
            ...
            if(url == pageId){
                ...
                for(let i = 0, l = $rootScope.imagelist.length; i < l; i++) {
                    for(let o = 0, s = $rootScope.imagelist[i].metadata.tags.length; o < s; o++){
                        switch($rootScope.imagelist[i].metadata.tag[o]){
                            case $rootScope.projectPage.settings.name:
                                $scope.project_imagelist.push($rootScope.imagelist[i]);
                                break;
                            default:
                                return;
                        }
