    ...
    var load_page = function(){
        for(let i = 0, l = $rootScope.pages.length; i < l && continue_looping == true; i++) {
            ...
            if(url == pageId){
                ...
                if($rootScope.imagelist.length > 0){
                    $scope.projectImages = filterFilter($rootScope.imagelist,$rootScope.projectPage.data.title);
                } else {
                    $rootScope.$on('images_loaded', function(event, args) {
                        $scope.projectImages = filterFilter($rootScope.imagelist,$rootScope.projectPage.data.title);
                    });
                }
